...
from app.providers.rentcast import RentCastApiError
...
async def refresh_listings_job() -> None:
    ...
    Provider API failures (missing key, quota exhausted, transient outage)
    are logged and skipped rather than raised — an uncaught exception here
    would kill the APScheduler job silently rather than just this one run.
    """
    ...
    try:
        ...
        logger.warning("refresh_job.provider_error", error=str(exc))
        return
